    ...
    async register(data){
        return new Promise(async (resolve, reject) => {
            try{ 
                ...
                if(!data.email.match(/^[a-zA-Z0-9]+(\.[a-zA-Z0-9]+)*@[a-zA-Z0-9-]+\.[a-zA-Z]{2,}$/)) resolve("Podany email nie jest niepoprawny!");
                ...
                if(!hasEmptyValue && !emailExists){
                    ...
                    resolve("Należy podać wszystkie dane!")
                }
            } 
            catch(error){
        ...
        return new Promise(async (resolve, reject) => {
            try{ 
                ...
                if(decoded.hasOwnProperty("message")){
                    console.log(decoded);
                    if(decoded.message == "invalid token") resolve("Wprowadzono niewłaściwy link!")
                    if(decoded.message == "jwt expired") resolve("Otrzymany link wygasł!")
                }
                else{
        ...
        return new Promise(async (resolve, reject) => {
            try{
                ...
                if(user){
                    ...
                    if(encrypted){
                        let token = this.createToken(passes.email, passes.password, "60m");
                        resolve({ "token": token });
                    }
                    else resolve("Podane hasło jest błędne!");
                }
